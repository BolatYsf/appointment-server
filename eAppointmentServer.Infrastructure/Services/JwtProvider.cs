...
{
    internal sealed class JwtProvider(IConfiguration configuration , IUserRoleRepository userRoleRepository ,RoleManager<AppRole> roleManager) : IJwtProvider
    {
        public async Task<string> CreateToken(AppUser user)
        {
            ...
            foreach (var userRole in appUserRoles)
            {
                ...
                if (role is not null)
                {
                    roles.Add(role); // role ekledim simdi jwt claim role ekleyecegim
                }
            }

            List<string?> stringRoles = roles.Select(s=> s.Name).ToList(); // role string list cevirmem gerekir claim verebilmek icin
